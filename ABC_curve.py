...
print("Arquivo em excel contendo apenas a tabela na seguinte ordem\n[ITEM] [DISCRIMINAÇÃO] [UNIDADE] [QUANTIDADE] [VALOR UNITÁRIO] [PARTICIPAÇÃO]")
maxNum = len(excelRaw)-1

df = excelRaw.drop(maxNum)
ind = np.arange(1, maxNum+1, 1)

df.insert(5,'VTC', df[df.columns[3]] * df[df.columns[4]])
df.insert(6,'PERCENTUAL', df['VTC'] / sum(df['VTC']) * 100)
